import *as PeliculasServices from '../../services/proyectos.services.js'
...
function findall(req, res){
    ...
    PeliculasServices.traerProyectos(filter)
...
function editById(req, res){
    ...
    .then(function(pelicula){
        if (pelicula) {
            res.status(200).json({messagge : "Proyecto editada con éxito."})
        }   else {
            res.status(404).json({messagge : "Proyecto no encontrada"})
        } 
    })
}
